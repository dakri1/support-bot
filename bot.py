...
def start(message):
    bot.send_message(message.chat.id, """👋🏻 Привет! Ты попал в бота технической поддержки @packsprivate_bot

Для того, чтобы обратиться в поддержку просто нажмите на кнопку 'Написать запрос', и напишите сообщение или пришлите изображение с вашим вопросом.""", parse_mode='html', reply_markup=markup.markup_main())
...
def get_new_request(message):
    ...
    if check_file != None:
        ...
        if str(request) == 'None':
            ...
            send_new_request_to_agents(req_id)
...
def send_new_request_to_agents(req_id):
    agents = core.get_agents()
    for agent in agents:
        bot.send_message(agent[0], f"Новый запрос под ID {req_id} создан, ожидает ответа")
